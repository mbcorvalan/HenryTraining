...
function crearClasePersona() {
	class Persona {
		...
		getFriends() {
			...
			// Tu código aca:
			let friends = this.amigos.map((amigo) => {
				return amigo.nombre
			})
			return friends
		}
		...
		getPromedioEdad() {
			...
			const sumaEdades = this.amigos.reduce(
				(acumulador, amigo) => acumulador + amigo.edad,
				0
			)
			const promedio = sumaEdades / this.amigos.length
